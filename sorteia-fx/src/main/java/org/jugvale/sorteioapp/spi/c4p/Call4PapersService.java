...
import org.jugvale.sorteioapp.spi.c4p.model.impl.Inscricao;
...
public class Call4PapersService implements SorteioAppService {
	...
	private final Properties c4pProps;
	...
	public List<Object> buscaValores() {
		...
		final List<Object> objetos = new ArrayList<>();
		pegaInscritosPresentesNoEvento(eventoID).forEach(objetos::add);
		return objetos;
	}
	
	private Collection<Inscricao> pegaInscritosPresentesNoEvento( final String eventoId ) {
	...
	private Collection<Inscricao> pegaInscritosNoEvento( final String eventoId ) {
